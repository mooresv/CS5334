 ...
 *
 * Compile:  gcc -g -Wall -o pth_tsp_dyn_rwlock pth_tsp_dyn_rwlock.c -lpthread
 *           Needs timer.h
 * Usage:    pth_tsp_dyn_rwlock <thread count> <matrix_file> <min split size>
 *
...
city_t home_town = 0;
tour_t best_tour;
pthread_rwlock_t best_tour_rwlock;
my_queue_t queue;
int queue_size;
int init_tour_count;
...
#  endif   
   ...
   pthread_rwlock_init(&best_tour_rwlock, NULL);
   Init_term();
...
#  endif
   ...
   pthread_rwlock_destroy(&best_tour_rwlock);
   Free_term();
   return 0;
}  /* main */
...
#     endif
      if (City_count(curr_tour) == n) {
         pthread_rwlock_rdlock(&best_tour_rwlock);
         best = Best_tour(curr_tour);
         pthread_rwlock_unlock(&best_tour_rwlock);
         if (best) {
#           ifdef PTSDEBUG
 ...
 */
void Update_best_tour(tour_t tour) {
   pthread_rwlock_wrlock(&best_tour_rwlock);
   if (Best_tour(tour)) {
