...
void main(int argc, char *argv[]){
  ...
  MPI_Cart_shift(comm_2d, 0, 1, &above, &below);
  coords[0] = (mycoords[0] + dims[0] - 1) % dims[0];
  coords[1] = (mycoords[1] + dims[1] - 1) % dims[1];
  MPI_Cart_rank(comm_2d, coords, &upper_left);
  coords[1] = (mycoords[1] + 1) % dims[1];
  MPI_Cart_rank(comm_2d, coords, &upper_right);
  coords[0] = (mycoords[0] + 1) % dims[0];
  MPI_Cart_rank(comm_2d, coords, &lower_right);
  coords[1] = (mycoords[1] + dims[1] - 1) % dims[1];
  MPI_Cart_rank(comm_2d, coords, &lower_left);
  ...
  //if (my2drank == 0) printf("Starting iterations\n");
  /*  Iterate */
  for(n=0; n<NSTEPS; n++){
   /* transfer data to ghost cells */
	...
	//if (my2drank == 0)printf("exchanging left and right columns\n");
        MPI_Sendrecv(&old[1][j2m], 1, column_type, right,  2, &old[1][0], 1, column_type, left, 2, comm_2d, &status);
        MPI_Sendrecv(&old[1][1], 1, column_type, left, 3, &old[1][j2], 1, column_type, right, 3, comm_2d, &status);
 
	//if (my2drank == 0) printf("exchanging corners\n");
        /* corners */
	MPI_Sendrecv(&old[1][1], 1, MPI_INT, upper_left, 10, &old[i2][j2], 1, MPI_INT, lower_right, 10, comm_2d, &status); 
        MPI_Sendrecv(&old[1][j2m], 1, MPI_INT, upper_right, 11, &old[i2][0], 1, MPI_INT, lower_left, 11, comm_2d, &status); 
        MPI_Sendrecv(&old[i2m][1], 1, MPI_INT, lower_left, 12, &old[0][j2], 1, MPI_INT, upper_right, 12, comm_2d, &status); 
        MPI_Sendrecv(&old[i2m][j2m], 1, MPI_INT, lower_right, 13, &old[0][0], 1, MPI_INT, upper_left, 13, comm_2d, &status);
      }
