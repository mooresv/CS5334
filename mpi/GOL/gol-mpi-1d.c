...
void main(int argc, char *argv[]){
  int i, j, n, im, ip, jm, jp, nsum, isum, myisum, isumloc, nprocs ,myid;
  int N;
  ...
  for(n=0; n<NSTEPS; n++){
    ...
    if(nprocs == 1){
      ...
      /* top and bottom rows */
      if(myid % 2 == 0){
        MPI_Send(&old[i2m][0], nj, MPI_INT, (myid+1) % nprocs,  0, MPI_COMM_WORLD);
        MPI_Recv(&old[i2][0],   nj, MPI_INT, (myid+1) % nprocs,  1, MPI_COMM_WORLD, &status);
        MPI_Send(&old[1][0],    nj, MPI_INT, (myid+nprocs-1) % nprocs,  1, MPI_COMM_WORLD);
        MPI_Recv(&old[0][0],    nj, MPI_INT, (myid+nprocs-1) % nprocs,  0, MPI_COMM_WORLD, &status);
        ...
        /* corners */
      if (myid == 0) {
        MPI_Send(&old[1][1],     1, MPI_INT, (nprocs-1), 10, MPI_COMM_WORLD);
        MPI_Recv(&old[0][0],     1, MPI_INT, (nprocs-1), 11, MPI_COMM_WORLD, &status);
        MPI_Send(&old[1][j2m],   1, MPI_INT, (nprocs-1), 12, MPI_COMM_WORLD);
        MPI_Recv(&old[0][j2],    1, MPI_INT, (nprocs-1), 13, MPI_COMM_WORLD, &status);
      }
      if (myid == (nprocs-1)) {
